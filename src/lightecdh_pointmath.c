...
void lightecdh_point_mul(bit x, bit y, const bit exp) {
  ...
  lightecdh_point_zero(tmpx, tmpy);
  lightecdh_point_zero(dummyx, dummyy);
  for (int i = (nb - 1); i >= 0; --i) {
    lightecdh_point_double(tmpx, tmpy);
    // Add point if bit(i) is set in exp
    if (lightecdh_bit_get(exp, i)) {
      lightecdh_point_add(tmpx, tmpy, x, y);
    }
  }

  lightecdh_point_copy(x, y, tmpx, tmpy);
}
...
void lightecdh_point_add(bit x1, bit y1, const bit x2, const bit y2) {
  extern int ecdh_a;
  if (!lightecdh_point_is_zero(x2, y2)) {
    if (lightecdh_point_is_zero(x1, y1)) {
      lightecdh_point_copy(x1, y1, x2, y2);
    } else {
      if (lightecdh_bit_equal(x1, x2)) {
        if (lightecdh_bit_equal(y1, y2)) {
          lightecdh_point_double(x1, y1);
        } else {
          lightecdh_point_zero(x1, y1);
        }
      } else {
...
int lightecdh_point_on_curve(const bit x, const bit y) {
  ...
  if (lightecdh_point_is_zero(x, y)) {
    return 1;
  } else {
